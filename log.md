...
### Day 35: December 19 2018

**Today's Progress**: I did the last project in Wes Bos course today and the biggest takeaways were the `setTimeout()` function and how we can remove and add classes that have a counter or event binded to them. `setTimeout(() => { hole.classList.remove("css-class"); }, time)` would be one example and time would be variable.
Another thing I learned was about the event method itself. A property of it is `isTrusted` which indicates if the event was invoked by the user or faked by some Javascript for example. Also I finished the JS Tip Calculator exercise. I used the new `...spread operator` and referenced to Wes Bos ES6 for everyone course. I learned how to spread an array as an argument into a function which then goes through all iterables of that one by one. It was useful for me cause I mapped the array with the bills in that function which then also iterated to its array. So I had 2 single values to work with. I then split the strings at the dollarsign and made sure I made these Numbers that I then could calculate with. I then later took that finished array, added the dollar sign back with a map thorugh the array and just a template literal to (pre-pend?? append) the dollar sign.
